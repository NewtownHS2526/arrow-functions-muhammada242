...
Use map() to create a new array with full names in the format
"FirstName LastName".
...
EXERCISE 2
----------
Given the array [3, 7, 2, 9, 5], use map() to create a new array where
each number is converted to a string and formatted as "Number: X".
...
EXERCISE 3
----------
Given the array of numbers [10, 25, 30, 45], use map() to create a new
array where each number is converted to a percentage string (e.g., 10
becomes "10%").
...
use map() to create a new array where each string's first letter of
each word is capitalized (e.g., "hello world" becomes "Hello World").
...
Use map() to create a new array with the total cost (price * quantity)
for each item.
